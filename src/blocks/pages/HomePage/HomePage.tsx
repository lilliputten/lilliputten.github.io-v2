...
import './HomePage.css';
// import './HomePage-rainbow-effects.css';

// Elems...
import HomePageAetherCanvas from './-AetherCanvas/HomePage-AetherCanvas';
// import HomePageFlakesCanvas from './-FlakesCanvas/HomePage-FlakesCanvas'; // UNUSED: Dynamic rainbow color effects
...
export default class HomePage extends React.Component<{}, IHomePageState> {
    ...
    return (
      <div className={cnHomePage({ready})}>
        <div className={cnHomePage('Effects', {ready})}>
          <HomePageAetherCanvas ready={ready} width={width} height={height} />
          {/* // UNUSED: Dynamic rainbow color effects...
          <HomePageFlakesCanvas ready={ready} width={width} height={height} />
          <div
            className={cnHomePage('Effect', {2: true, ready})}
            style={this.getEffectStyle(2)}
          />
          <div
            className={cnHomePage('Effect', {1: true, ready})}
            style={this.getEffectStyle(1)}
          />
          */}
        </div>
      </div>
  ...
  private getWindowSizes(): {width: number, height: number, gap: number} {
    // NOTE: Window sizes was used for flakes canvas (see `src/blocks/pages/HomePage/-FlakesCanvas`)
